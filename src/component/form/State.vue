...
<script>
import brStates from './supportData/brStates'
...
export default {
  ...
  computed: {
    ...
    filteredOptions () {
      if (!this.searchValue) return this.stateOptions

      const needle = this.normalizeString(this.searchValue)
      const options = this.stateOptions.filter(e => this.normalizeString(e.label || '').indexOf(needle) > -1)
